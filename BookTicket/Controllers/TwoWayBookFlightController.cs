...
namespace BookTicket.Controllers
{
    [Route("[controller]")]
    [ApiController]
    public class TwoWayBookFlightController : ControllerBase
    {
        ...
        public IActionResult Post([FromBody] PostRequestedDetails requestedDetails)
        {
            if (requestedDetails != null)
            {
                ...
                using (UserSideContext ctx = new UserSideContext())
                {
                    ...
                    //round Trip
                    if (requestedDetails.SentRoundUniqueId != "")
                    {
                        FlightsSchedules roundflights = ctx.FlightsSchedules?.Where(x => x.UserName == requestedDetails.UserName &&
                                                                                    x.EntryId == requestedDetails.SentRoundUniqueId)
                                                                                    .FirstOrDefault();
                        string pnrroundNumber = _bookingSystem.PNRNumberGeneration(roundflights.FlightNumber);
